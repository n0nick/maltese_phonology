...
Looking at table (5), this constraint would indeed explain the deletions in examples (b), but alone it would fail to explain why the vowel wasn't deleted in examples (1a), resulting for example in [\textipa{*\textsl{h5tf}}].
...
So, our constraints ranking so far:
...
Yet the actual output turns out to be \textsl{\textipa{[ht5ft]}}, which violates {\sc *CC} and is a result of deleting the first \textsl{\textipa{[5]}} vowel that we assume was originally the stressed vowel.
\\
