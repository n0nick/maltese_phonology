...
Let's consider the Maltese subject suffixes for the 2\super{nd} person singular subjects; the empty suffix \textipa{+\O} for 'he' and the suffix \textipa{+5t} for 'she' \cite{brame1974}.
...
This new data set forces us to rethink our analysis. While it's possible to formulate a different rule or constraint to explain the deletion in (c), both vowel deletions appear to have some common motivation, and so it would be ideal if we could formulate some unified constraint that would explain these, and perhaps other, alternations.
...
Examples (1d) and (1e) contradict any attempt to explain the vowel alternations by structural constraints alone. While the URs of both examples offer the same possible product \textipa{[h5.t5f.n5]}, it is only preserved in (1e), with a 1-sg subject suffix and a 1-pp object suffix. A subject suffix alone did not help (1d) avoid the vowel deletion we witnessed in all other examples.
\\
...
We will argue that this same difference plays an important part in the Maltese stress system, and see how this system's cyclicity can make the difference between executing a vowel deletion and skipping it.
