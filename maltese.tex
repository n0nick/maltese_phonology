...
\paragraph* {(9)} {{\sc Head(PrWd)} - Each prosodic word has a unique head \cite[78]{mccarthy2002thematic} and therefore, it has stress.}
\\
...
\paragraph* {(11)} {{\sc Head(PrWd) >> *V[-stress], *CC}}
...
\multicolumn{3}{|c||}{/\textipa{h5t5f+\O}/} & {\sc Head(PrWd)} & {\sc MaxStem} & {\sc *CC} & {\sc *V[-stress]} \\ \hline\hline
...
\paragraph*{(13)} {\sc Head(PrWd) >> *CC >> *V[-stress]}
...
The ranking we have established so far seems to be fairly straightforward and to explain the first two examples quite nicely.
